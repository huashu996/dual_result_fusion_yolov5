...
parser.add_argument('--sub_image2', default='/pub_t', type=str,
    help='The image topic to subscribe.')
parser.add_argument('--sub_image3', default='/pub_t', type=str, #深度
    help='The image topic to subscribe.')
...
#3.3 获取深度图像的时间戳和格式转化
#×××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××
def image3_callback(image):
    global image3_stamp, image3_frame
    image3_lock.acquire()
    image3_stamp = get_stamp(image.header)
    image3_frame = np.frombuffer(image.data, dtype=np.uint8).reshape(image.height, image.width, -1)
    image3_lock.release()
...
def timer_callback(event):
    ...
    global image3_stamp, image3_frame
    image3_lock.acquire()
    cur_stamp3 = image3_stamp
    cur_frame3 = image3_frame.copy()
    image3_lock.release()
    ...
    # 5.3双目估计距离
    #×××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××
    locations = estimate(boxes,cur_frame3)#获得目标框的世界坐标系
    indices = [i for i in range(len(locations)) if locations[i] > 0 and locations[i] < 200]
    ...
    # 5.4画检测框
    #×××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××
    for i in reversed(np.argsort(locations)):#reversed()函数用于对可迭代对象中的元素进行反向排列
...
def estimate(boxes,img):#单目估计距离
        ...
        if boxes.shape[0] > 0: #判断目标个数是否大于1
            for box in boxes:
                ...
                x3, y3 = int((box[0]+box[2])/2) ,int((box[1]-box[3])/2)
                x4, y4 = int((box[0]-box[2])/2) ,int((box[1]-box[3])/2)
                x5, y5 = int((box[0]-box[2])/2) ,int((box[1]+box[3])/2)
                z1 = gray[y1, x1]
                z2 = gray[y2, x2]
                z3 = gray[y3, x3]
                z4 = gray[y4, x4]
                z5 = gray[y5, x5]
                z=(int(z1)+int(z2)+int(z3)+int(z4)+int(z5))/5
                locations.append((z))
        return locations
...
if __name__ == '__main__':
    ...
    if args.indoor:
        ...
        if args.modality.lower() == 'rgb':
            ...
            detector2 = Yolov5Detector(weights='weights/50000lrwtiny.pt')
        else:
            raise ValueError("The modality must be 'RGB', 'T' or 'RGBT'.")
    # 三、进入回调函数
    #×××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××
    ...
    image3_stamp, image3_frame = None, None
    ...
    rospy.Subscriber(args.sub_image2, Image, image2_callback, queue_size=1,
        buff_size=52428800)
    #深度图回调函数
    rospy.Subscriber(args.sub_image3, Image, image3_callback, queue_size=1,
        buff_size=52428800)
